...
public class TrieSET implements Iterable<String> {
    ...
    private Node get(Node x, String key, int d) {
        ...
        char c = key.charAt(d);
        return get(x.next[charOf(c)], key, d+1);
    }
    ...
    private Node add(Node x, String key, int d) {
        ...
        if (d == key.length()) {
            ...
            char c = key.charAt(d);
            x.next[charOf(c)] = add(x.next[charOf(c)], key, d+1);
        }
        return x;
    }
    ...
    private void collect(Node x, StringBuilder prefix, Queue<String> results) {
        ...
        for (char c = 0; c < R; c++) {
            prefix.append(c);
            collect(x.next[charOf(c)], prefix, results);
    ...
    private void collect(Node x, StringBuilder prefix, String pattern, Queue<String> results) {
        ...
        if (c == '.') {
            ...
            prefix.append(c);
            collect(x.next[charOf(c)], prefix, pattern, results);
            prefix.deleteCharAt(prefix.length() - 1);
        }
    }
    ...
    private int longestPrefixOf(Node x, String query, int d, int length) {
        ...
        return longestPrefixOf(x.next[charOf(c)], query, d+1, length);
    }
    ...
    private Node delete(Node x, String key, int d) {
        ...
        if (d == key.length()) {
            ...
            char c = key.charAt(d);
            x.next[charOf(c)] = delete(x.next[charOf(c)], key, d+1);
        }

        // remove subtrie rooted at x if it is completely empty
        if (x.isString) return x;
        for (int c = 0; c < R; c++)
            if (x.next[charOf(c)] != null)
                return x;
        return null;
    }
